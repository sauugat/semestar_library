...
// Class Reps / Teacher / Admin accounts marked with role: "admin"
const students = [
...
const insert = db.prepare('INSERT OR REPLACE INTO students (studentId, name, passwordHash, role) VALUES (?, ?, ?, ?)');

for (const s of students) {
  const hash = bcrypt.hashSync(s.password, 10);
  const role = s.role || 'student';
  insert.run(s.studentId, s.name, hash, role);
